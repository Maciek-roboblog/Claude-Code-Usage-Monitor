...
- **`claude-opus-5` no longer billed at legacy Opus 3 rates**: `claude-opus-5` was previously mis-normalized to the legacy `claude-3-opus` pricing key and billed at $15/$75 instead of the correct $5/$25 — a 3x cost overcount. Model-name normalization now only collapses actual Claude 3-era names to legacy pricing keys; 4.x/5-era and unversioned model ids pass straight through instead of being folded into a Claude 3 bucket.
...
- **Claude 5 model family recognized**: `claude-opus-5`, `claude-sonnet-5`, `claude-fable-5`, and `claude-mythos-5` (Project Glasswing) are now recognized model ids with correct current pricing (Opus 5 $5/$25, Sonnet 5 $3/$15 sticker — its time-boxed $2/$10 introductory rate through 2026-08-31 is not modeled, Fable/Mythos $10/$50) instead of being mis-bucketed as legacy or "Other". A "Fable" family now appears alongside Sonnet/Opus/Haiku in the per-model usage bar and in the snapshot model distribution. Family bucketing is centralized in a single `model_family()` helper (`core/models.py`) shared by pricing, the usage bar, and snapshots, so recognizing a future model launch is a one-place change; an unrecognized future family name now falls back to current-rate pricing instead of a legacy rate.
- **Custom plan limits labeled as auto-estimates**: the default custom plan's P90-derived token, cost, and message limits are now called out in the UI with an `(auto P90)` qualifier rather than reading like fixed, authoritative numbers. The "cost limit will be exceeded" warning is therefore a forecast against your own historical P90 cost, not a billing threshold.
- **"Time to Reset" bar aligned with the other metric bars**: the stopwatch icon (U+23F1 + VS16) is narrow in Unicode width tables but renders 1 or 2 cells depending on the terminal, skewing the row by one column; replaced with the unambiguously wide hourglass (U+23F3).
