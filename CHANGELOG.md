...
- **Warehouse retention no longer wipes long-horizon stores**: a retention window longer than the representable calendar (e.g. `--warehouse-retention-days 3650`) overflowed the cutoff date math and pruned every record on each write, silently emptying the warehouse; the overflow is now detected and all records are kept instead.
- **`--warehouse` now persists from daily/monthly views**: `--view daily|monthly --warehouse` was silently ignored because the table views never touched the warehouse; they now upsert every loaded entry (plus detected limit events) best-effort, warning instead of failing on write errors. This is also the way to backfill full local history, since the live monitor only persists the last 8 days (`hours_back=192`).
