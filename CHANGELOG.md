...
- **Claude 5 model family recognized**: `claude-opus-5`, `claude-sonnet-5`, `claude-fable-5`, and `claude-mythos-5` (Project Glasswing) are now recognized model ids with correct current pricing (Opus 5 $5/$25, Sonnet 5 $3/$15 sticker, Fable/Mythos $10/$50) instead of being mis-bucketed as legacy or "Other". A "Fable" family now appears alongside Sonnet/Opus/Haiku in the per-model usage bar and in the snapshot model distribution. Family bucketing is centralized in a single `model_family()` helper (`core/models.py`) shared by pricing, the usage bar, and snapshots, so recognizing a future model launch is a one-place change; an unrecognized future family name now falls back to current-rate pricing instead of a legacy rate.
