...
check_python_imports() {
    ...
    # Check usage_analyzer import
    if ! python -c "import usage_analyzer" 2>/dev/null; then
        log_error "Failed to import usage_analyzer module"
        ((import_errors++))
    fi
    
    # Check api import
    if ! python -c "from usage_analyzer.api import analyze_usage" 2>/dev/null; then
