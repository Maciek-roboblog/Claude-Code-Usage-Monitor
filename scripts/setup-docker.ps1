...
function Find-ClaudeData {
    ...
    foreach ($path in $claudePaths) {
        if (Test-Path $path) {
            $jsonlFiles = Get-ChildItem -Path $path -Filter "*.jsonl" -Recurse -Depth 3 -ErrorAction SilentlyContinue
...
function Test-Installation {
    ...
    # Health check test
    try {
        $testResult = docker run --rm -v "${ClaudeDataPath}:/data:ro" --entrypoint python "${ImageName}:latest" -c "from claude_monitor.data.analysis import analyze_usage; result = analyze_usage(); print('✅ Test passed: {} blocks found'.format(len(result.get('blocks', []))))"
